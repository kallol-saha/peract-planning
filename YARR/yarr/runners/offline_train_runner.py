...
class OfflineTrainRunner():
    ...
    def _step(self, i, sampled_batch):
        ...
        return total_losses, update_dict
    ...
    def start(self):
        ...
        for i in range(start_iter, self._iterations):
            ...
            t = time.time()
            loss, update_dict = self._step(i, batch)
            ...
            if self._rank == 0:
                ...
                if i % self._save_freq == 0 and self._weightsdir is not None:
                    ...
                    if update_dict.get('voxel_grid', None) is not None:
                        voxel_grid = update_dict['voxel_grid']
                        action_voxels = update_dict['action_voxels']
                        action_colors = update_dict['action_colors']
                        # Save the voxel grid and action voxels:
                        np.savez(os.path.join(self._weightsdir, str(i), f'voxel_grid_{i}.npz'),
                                 total_losses=loss,
                                 voxel_grid=voxel_grid.cpu().detach().numpy(), 
                                 action_voxels=action_voxels.cpu().detach().numpy(), 
                                 action_colors=action_colors.cpu().detach().numpy())
